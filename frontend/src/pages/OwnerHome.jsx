import React, { useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import { useNavigate } from "react-router-dom";
import { Link } from "react-router-dom";
import { logoutUser } from "../redux/actions/authActions";
import {
  FaUserCircle,
  FaSignOutAlt,
  FaUser,
  FaChevronDown,
  FaTimes,
} from "react-icons/fa";

const plans = [
  {
    name: "Free",
    price: "₹0",
    features: ["Basic PG management", "Add up to 2 PGs", "Limited support"],
  },
  {
    name: "Pro",
    price: "₹499/mo",
    features: ["Unlimited PGs", "Advanced analytics", "Priority support"],
  },
  {
    name: "Enterprise",
    price: "Contact Us",
    features: ["Custom solutions", "Dedicated manager", "API access"],
  },
];

const OwnerHome = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const user = useSelector((state) => state.auth.user);
  const [menuOpen, setMenuOpen] = useState(false);

  const handleLogout = () => {
    dispatch(logoutUser());
    navigate("/");
  };

  const [showProfile, setShowProfile] = useState(false);

  const handleProfile = () => {
    setMenuOpen(false);
    setShowProfile(true);
  };

  const handleDashboard = () => {
    navigate("/auth/dashboard");
  };

  return (
    <div>
      {/* Navbar */}
      <nav className="flex items-center justify-between bg-white px-8 py-4 shadow-md border-b border-purple-200">

        <Link to="" className="flex items-center gap-2">
          <h1 className="text-4xl font-extrabold mb-4 text-purple-600 tracking-tight text-center">
            Ghar Mitra 🏡🤝
          </h1>
        </Link>
        <div className="relative">
          <button
            className="flex items-center space-x-2 focus:outline-none"
            onClick={() => setMenuOpen((open) => !open)}
          >
            <FaUserCircle className="text-3xl text-purple-600" />
            <FaChevronDown className="text-gray-500 ml-1" />
          </button>
          {menuOpen && (
            <div className="absolute right-0 mt-2 w-40 bg-white border border-gray-200 rounded-xl shadow-lg z-50">
              <button
                className="w-full flex items-center px-4 py-2 hover:bg-purple-100 text-gray-800"
                onClick={handleProfile}
              >
                <FaUser className="mr-2" /> Profile
              </button>
              <button
                className="w-full flex items-center px-4 py-2 hover:bg-purple-100 text-gray-800"
                onClick={handleLogout}
              >
                <FaSignOutAlt className="mr-2" /> Logout
              </button>
            </div>
          )}
        </div>
      </nav>

      {/* Hero Section */}
      <section className="flex flex-col items-center justify-center py-20 bg-gradient-to-b from-purple-100 to-white">
        <h1 className="text-5xl font-extrabold text-purple-700 mb-4 text-center">
          Welcome, {user?.username || "PG Owner"}!
        </h1>
        <p className="text-lg text-gray-700 mb-8 text-center max-w-2xl">
          Effortlessly manage your PGs, track students, handle payments, and
          grow your business with our all-in-one platform.
        </p>
        <button
          className="bg-purple-700 hover:bg-purple-800 text-white font-bold py-4 px-8 rounded-xl text-xl shadow-md transition-all mb-6"
          onClick={handleDashboard}
        >
          Manage Your Dashboard
        </button>
      </section>

      {/* Features Section */}
      <section className="py-40 bg-white">
        <div className="max-w-6xl mx-auto px-4">
          <h2 className="text-3xl font-bold text-center text-purple-700 mb-10">
            Features
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="bg-purple-50 p-8 rounded-xl shadow">
              <h3 className="text-xl font-bold mb-2 text-purple-600">
                Student Management
              </h3>
              <p className="text-gray-700">
                Add, edit, and track all your PG students in one place with
                ease.
              </p>
            </div>
            <div className="bg-purple-50 p-8 rounded-xl shadow">
              <h3 className="text-xl font-bold mb-2 text-purple-600">
                PG Listings
              </h3>
              <p className="text-gray-700">
                Register and manage multiple PG properties with detailed
                information.
              </p>
            </div>
            <div className="bg-purple-50 p-8 rounded-xl shadow">
              <h3 className="text-xl font-bold mb-2 text-purple-600">
                Payments & Analytics
              </h3>
              <p className="text-gray-700">
                Track payments, view analytics, and get insights to optimize
                your business.
              </p>
            </div>
          </div>
        </div>
      </section>

      {/* Plans Section */}
      <section className="py-16 bg-gradient-to-b from-white to-purple-50">
        <div className="max-w-6xl mx-auto px-4">
          <h2 className="text-3xl font-bold text-center text-purple-700 mb-10">
            Subscription Plans
          </h2>
          <div className="flex flex-col md:flex-row items-center justify-center gap-8">
            {plans.map((plan) => (
              <div
                key={plan.name}
                className="bg-white rounded-xl shadow-lg border border-purple-200 p-8 flex-1 max-w-xs text-center"
              >
                <h3 className="text-xl font-extrabold text-purple-700 mb-2">
                  {plan.name}
                </h3>
                <div className="text-3xl font-bold mb-4">{plan.price}</div>
                <ul className="mb-6 text-gray-700 text-left list-disc list-inside">
                  {plan.features.map((feature) => (
                    <li key={feature}>{feature}</li>
                  ))}
                </ul>
                <button className="bg-purple-600 hover:bg-purple-700 text-white py-2 px-6 rounded-xl font-semibold transition-all">
                  {plan.name === "Free" ? "Current Plan" : "Choose Plan"}
                </button>
              </div>
            ))}
          </div>
        </div>
      </section>
      {/* {showProfile && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-white/60 backdrop-blur-sm">
          <div className="relative">
            <button
              className="absolute top-2 right-2 text-gray-500 hover:text-gray-700 bg-white rounded-full p-2 shadow"
              onClick={() => setShowProfile(false)}
            >
              <FaTimes className="text-gray-500" />
            </button>
            <PgOwnerProfile />
          </div>
        </div>
      )} */}

      {/* Profile Modal */}
      {showProfile && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-white/60 backdrop-blur-sm">
          <div className="relative">
            <button
              className="absolute top-2 right-2 text-gray-500 hover:text-gray-700 bg-white rounded-full p-2 shadow"
              onClick={() => setShowProfile(false)}
            >
              <FaTimes className="text-gray-500" />
            </button>
            {/* You can use a StudentProfile component here if you have one */}
            <div className="p-8 bg-white rounded-2xl shadow-xl border border-gray-200 min-w-[300px]">
              <h2 className="text-2xl font-bold text-center mb-4 text-purple-700">
                Owner Profile
              </h2>
              <div className="space-y-3">
                <div className="flex flex-col md:flex-row md:items-center gap-2">
                  <span className="font-semibold text-gray-700 w-32">
                    Username:
                  </span>
                  <span className="text-lg text-gray-900">
                    {user?.username || "-"}
                  </span>
                </div>
                <div className="flex flex-col md:flex-row md:items-center gap-2">
                  <span className="font-semibold text-gray-700 w-32">
                    Email:
                  </span>
                  <span className="text-lg text-gray-900">
                    {user?.email || "-"}
                  </span>
                </div>
                {/* Add more fields if needed */}
              </div>
            </div>
          </div>
        </div>
      )}
    </div>
  );
};

export default OwnerHome;
