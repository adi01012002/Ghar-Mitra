...
import { FaUserCircle, FaChevronDown, FaUser, FaSignOutAlt, FaMoneyBill, FaComments, FaUserAlt, FaTimes } from "react-icons/fa";

const StudentDashboard = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const user = useSelector((state) => state.auth.user);
  const { profile } = useSelector((state) => state.students);
    ...
    <div className="min-h-screen bg-gradient-to-b from-purple-100 to-white">
      ...
      <nav className="flex items-center justify-between bg-white px-8 py-4 shadow-md border-b border-purple-200">
        <Link to="/student-home" className="flex items-center gap-2">
          <h1 className="text-4xl font-extrabold mb-4 text-purple-600 tracking-tight text-center">Ghar Mitra 🏡🤝</h1>
        </Link>
      ...
      <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <aside className="md:col-span-1 bg-white rounded-xl shadow-lg border border-purple-200 p-6 flex flex-col gap-4 mb-8 md:mb-0">
          <button className="flex items-center gap-2 py-2 px-4 rounded-lg hover:bg-purple-50 text-purple-700 font-semibold" onClick={() => handleNavigation(`/student/${user?._id}/profile`)}>
            <FaUserAlt /> My Profile
          </button>
          <button className="flex items-center gap-2 py-2 px-4 rounded-lg hover:bg-purple-50 text-purple-700 font-semibold" onClick={() => handleNavigation(`/student/${user?._id}/payments`)}>
            <FaMoneyBill /> My Payments
          </button>
          <button className="flex items-center gap-2 py-2 px-4 rounded-lg hover:bg-purple-50 text-purple-700 font-semibold" onClick={() => handleNavigation(`/student/${user?._id}/complaints`)}>
            <FaComments /> My Complaints
          </button>
        </aside>
        {/* Main Content */}
        <main className="md:col-span-3 bg-white rounded-xl shadow-lg border border-purple-200 p-8">
          <h2 className="text-3xl font-extrabold mb-6 text-purple-600 tracking-tight">Student Dashboard Overview</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="bg-purple-50 p-6 rounded-xl shadow">
              <h3 className="text-xl font-bold mb-2 text-purple-600">Student Profile</h3>
              <p className="text-gray-700">View and update your profile information easily.</p>
            </div>
            <div className="bg-purple-50 p-6 rounded-xl shadow">
              <h3 className="text-xl font-bold mb-2 text-purple-600">Payments</h3>
              <p className="text-gray-700">Track your payments and view payment history.</p>
            </div>
            <div className="bg-purple-50 p-6 rounded-xl shadow">
              <h3 className="text-xl font-bold mb-2 text-purple-600">Complaints</h3>
              <p className="text-gray-700">Raise and track complaints for quick resolution.</p>
            </div>
            <div className="bg-purple-50 p-6 rounded-xl shadow">
